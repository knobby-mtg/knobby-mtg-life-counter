...
static void refresh_life_digits()
{
    ...
    int tens = (v / 10) % 10;
    int ones = v % 10;
    lv_color_t c = get_life_color(v);

    if (life_container != NULL) {
        lv_obj_align(life_container, LV_ALIGN_CENTER, (v >= 100) ? -18 : 0, -6);
    }
    if (life_hitbox != NULL) {
        lv_obj_align(life_hitbox, LV_ALIGN_CENTER, (v >= 100) ? -18 : 0, -8);
    }

    if (v >= 100) {
        lv_obj_set_pos(digit_box_hundreds, 10, 0);
        ...
        lv_obj_set_pos(digit_box_tens, 45, 0);
        ...
        lv_obj_set_pos(digit_box_ones, 80, 0);
        lv_obj_clear_flag(digit_box_ones, LV_OBJ_FLAG_HIDDEN);
...
static void refresh_select_ui()
{
    ...
    for (i = 0; i < ENEMY_COUNT; i++) {
        lv_label_set_text(label_enemy_name[i], enemies[i].name);
...
static void refresh_damage_ui()
{
    ...
    lv_label_set_text(label_damage_title, enemies[selected_enemy].name);
...
{
    static const uint32_t base_colors[MULTIPLAYER_COUNT] = {0x1B6ED1, 0x238847, 0xB11B2F, 0x7A8386};
    static const uint32_t active_colors[MULTIPLAYER_COUNT] = {0x2F8EFF, 0x35B85F, 0xFF3B57, 0xB3BEC2};
    ...
    for (i = 0; i < MULTIPLAYER_COUNT; i++) {
        if (multiplayer_quadrants[i] != NULL) {
            lv_obj_set_style_bg_color(
                ...
                lv_color_hex((i == multiplayer_selected) ? active_colors[i] : base_colors[i]),
                0
            );
...
{
    refresh_multiplayer_ui();
    lv_scr_load(screen_multiplayer);
}
...
static void open_multiplayer_menu_screen(int player_index)
{
    ...
    refresh_multiplayer_menu_ui();
    lv_scr_load(screen_multiplayer_menu);
}

static void open_multiplayer_name_screen(void)
{
    refresh_multiplayer_name_ui();
    lv_scr_load(screen_multiplayer_name);
}

static void open_multiplayer_cmd_select_screen(void)
{
    refresh_multiplayer_cmd_select_ui();
    lv_scr_load(screen_multiplayer_cmd_select);
}
...
static void open_multiplayer_cmd_damage_screen(int target_index)
{
    ...
    refresh_multiplayer_cmd_damage_ui();
    lv_scr_load(screen_multiplayer_cmd_damage);
}
...
static void open_multiplayer_all_damage_screen(void)
{
    multiplayer_all_damage_value = 0;
    refresh_multiplayer_all_damage_ui();
    lv_scr_load(screen_multiplayer_all_damage);
}
...
static void build_main_screen()
{
    ...
    life_hitbox = make_plain_box(screen_main, 252, 188);
    ...
    life_container = make_plain_box(screen_main, 220, 112);
...
static void build_damage_screen()
{
    ...
    lv_label_set_text(label_damage_title, "Itze");
